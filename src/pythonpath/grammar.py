...
class GrammarChecker(
    ...
    def doProofreading(
        ...
        res = divvun_runtime.bundle_run_pipeline(
            ctypes.c_void_p(bundle).value, text.encode("utf-8")
        )
        logging.info("Pipeline finished")

        bytes = ctypes.c_char_p(res).value
        logging.info(bytes)
        grammarErrors = []
        if bytes:
            ...
            for err in data["errs"]:
                grammarErr = uno.createUnoStruct(
